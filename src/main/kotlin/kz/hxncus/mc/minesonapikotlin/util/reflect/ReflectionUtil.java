package kz.hxncus.mc.minesonapikotlin.util.reflect;
...
public class ReflectionUtil {
	...
	public <T> boolean setFieldValue(@NonNull final Object object, @NonNull final String fieldName, final T value) {
		try {
			this.getCachedField(object.getClass(), fieldName).set(object, value);
			return true;
	...
	private Field getCachedField(@NonNull final Class<?> clazz, @NonNull final String fieldName) {
		Map<String, Field> classFields = this.FIELD_CACHE.get(clazz);
		if (classFields == null) {
			classFields = new ConcurrentHashMap<>(8);
			final Map<String, Field> existingFields = this.FIELD_CACHE.putIfAbsent(clazz, classFields);
	...
	public Object getFieldValue(@NonNull final Class<?> clazz, @NonNull final String name, @NonNull final Object object) {
		final Field field = this.getDeclaredField(clazz, name);
		if (field == null)
			throw new IllegalArgumentException("Class must have a field named: " + name);
		return this.getFieldValue(field, object);
	}
	...
	public Class<?> getNMSClass(@NonNull final String classPath) {
		try {
			return Class.forName("net.minecraft.server." + this.getVersion() + "." + classPath);
		} catch (final ClassNotFoundException e) {
			return null;
		}
	}
	...
	public Class<?> getObcClass(@NonNull final String classPath) {
		try {
			return Class.forName("org.bukkit.craftbukkit." + this.getVersion() + "." + classPath);
		} catch (final ClassNotFoundException e) {
			return null;
		}
	}
